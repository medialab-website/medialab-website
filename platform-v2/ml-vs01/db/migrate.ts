...
const TEMPORARY_DOWNLOAD_CENTER_PACKET_MIGRATION = '0018_temporary_download_center_external_sharing_foundation.sql';
...
const TEMPORARY_DOWNLOAD_CENTER_PUBLIC_FUNCTIONS = [
  'medialab_core.create_temporary_download_center(text, text, uuid, integer, text, jsonb, text, jsonb)',
  'medialab_core.update_temporary_download_center_selection(text, text, uuid, bigint, jsonb, text)',
  'medialab_core.replace_temporary_download_center(text, text, uuid, bigint, integer, text, jsonb, text, jsonb)',
  'medialab_core.revoke_temporary_download_center(text, text, uuid, bigint, text)',
  'medialab_core.get_temporary_download_center(text, uuid)',
  'medialab_core.evaluate_temporary_download_center_policy(text, uuid)',
  'medialab_core.get_temporary_download_center_activity(text, uuid)'
];

const TEMPORARY_DOWNLOAD_CENTER_ACCESS_GATEWAY_PUBLIC_FUNCTIONS = [
  'medialab_core.issue_temporary_download_center_access_credential(text, text, uuid, bigint, text, jsonb)',
  'medialab_core.rotate_temporary_download_center_access_credential(text, text, uuid, integer, text, jsonb)',
  'medialab_core.revoke_temporary_download_center_access_credential(text, text, uuid, integer, text, jsonb)',
  'medialab_core.evaluate_temporary_download_center_gateway_access(uuid, text, text, text, uuid, jsonb)',
  'medialab_core.get_temporary_download_center_access_credential_history(text, uuid)',
  'medialab_core.get_temporary_download_center_gateway_history(text, uuid)'
];

const DISPOSABLE_DELIVERY_SURFACE_PUBLIC_FUNCTIONS = [
  'medialab_core.get_temporary_download_center_delivery_manifest(uuid, text, text, jsonb)'
];
...
export async function runMigrations(options: MigrateOptions): Promise<MigrationResult> {
  ...
  if (!client) {
    ...
    client = new pg.Client({
      host: options.host || process.env.PGHOST || '/tmp/mlvs01-p02m15c-pg',
      port: options.port || (process.env.PGPORT ? parseInt(process.env.PGPORT, 10) : 55444),
      database: options.database || process.env.PGDATABASE || 'medialab_p02m15c_test',
      user: migrationUser,
    ...
    for (const file of sqlFiles) {
      ...
      if (appliedLedger.has(file)) {
        ...
        try {
          ...
          if (file === TEMPORARY_DOWNLOAD_CENTER_PACKET_MIGRATION) {
            await applyRuntimePrivilegePolicy(client, runtimeUser!, true, true, true, true, true, true, true, true, true, true, true, true, true, true, true);
            authorityPolicyApplied = true;
          }
          if (file === TEMPORARY_DOWNLOAD_CENTER_ACCESS_GATEWAY_PACKET_MIGRATION) {
            await applyRuntimePrivilegePolicy(client, runtimeUser!, true, true, true, true, true, true, true, true, true, true, true, true, true, true, true, true);
            authorityPolicyApplied = true;
          }
          if (file === DISPOSABLE_DELIVERY_SURFACE_PACKET_MIGRATION) {
            await applyRuntimePrivilegePolicy(client, runtimeUser!, true, true, true, true, true, true, true, true, true, true, true, true, true, true, true, true, true);
            authorityPolicyApplied = true;
          }
          await client.query(
...
if (scriptPath && currentPath === scriptPath) {
  const targetDb = process.env.PGDATABASE || 'medialab_p02m15c_test';
