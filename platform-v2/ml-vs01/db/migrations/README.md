...
5. **Domain Migration Ownership**
   ...
   * `0013_capture_session_ingest_custody_foundation.sql` is the bounded P02-M10-A additive migration for provider-neutral Capture Sessions, one-way Job assignment, multiple intake sources, stable pre-asset items, append-only verification/custody/duplicate evidence, and controlled promotion into accepted immutable original assets.
   * `0014_media_cull_workspace_selected_media_evidence_foundation.sql` is the bounded P02-M11-A additive migration for one-Job PHOTO/VIDEO Cull Workspaces, exact immutable ORIGINAL candidate inventory, explicit sealing, append-only decisions and corrections, deterministic selected-media manifests, and immutable successor recull evidence.
   * `0015_editor_handoff_returned_media_intake_foundation.sql` is the bounded P02-M12-A additive migration for exact selected-version editor handoffs, separate PHOTO/VIDEO lanes, deterministic outbound manifests, partial and repeated returns, explicit returned-media matching outcomes, immutable returned versions and lineage, and reconciliation-aware current projections.
   * `0016_returned_editor_review_final_source_decision_foundation.sql` is the bounded P02-M13-A additive migration for sealed returned-editor human review cycles, per-item decisions, revision and Quick Edit routing, append-only correction, and exact final-source designations that reuse immutable media-version law.
   * `0017_publication_delivery_entitlement_foundation.sql` is the bounded P02-M14-A additive migration for provider-neutral publication identity over exact final-source versions, deterministic placements, bounded financial-eligibility evidence, PREVIEW/DOWNLOAD entitlement evaluation, authenticated subject authority, revocable grants, invalidation, and access re-evaluation.
   * `0018_temporary_download_center_external_sharing_foundation.sql` is the bounded P02-M15-A additive migration for fixed Temporary Download Center snapshots, immutable versioned selections/items, expiration, replacement, revocation, current sharing-policy evaluation, and future-compatible access observations without a public credential.
   * `0019_temporary_download_center_access_credential_gateway_foundation.sql` is the bounded P02-M15-C additive migration for one-time server-generated access secrets, nonreversible verifier evidence, rotation/revocation, shared current-policy evaluation, anonymous OPEN/DOWNLOAD authorization decisions, and the trusted append-only observation writer boundary.
   * `0020_disposable_delivery_surface_local_fixture_foundation.sql` is the bounded P02-M15-C local-fixture projection for deterministic delivery-manifest proof without provider or byte movement.
   * `0021_provider_neutral_file_backed_disposable_delivery_foundation.sql` is the bounded P02-M15-D internal file-backed delivery-source resolution foundation without network retrieval or customer exposure.
   * `0022_organization_records_dashboard_audited_export_foundation.sql` is the bounded P02-M15-E additive migration for fail-closed organization-funded versus personal-order classification, private-by-default personal records, explicit personal-owner non-financial summary sharing/revocation, deterministic internal records projections, immutable audited export snapshots and ordered membership, and append-only access evidence. It creates no trusted-billing activation, payment ledger, customer-facing dashboard authority, media authority, or export-file delivery.
   * `0023_runtime_intake_reconciliation_commands.sql` is the bounded P02-M16-E additive runtime command surface for explicit intake reconciliation without provider contact.
...
6. **Owner and Runtime Role Separation**
   ...
   * Raw manual execution of migration `0011` alone is likewise incomplete. The canonical runner grants only twelve controlled P02-M08-A media commands/projections, keeps all media helpers owner-only, grants no direct table or sequence authority, and grants no provider credential, ingest, upload, download, processing, delivery, or production capability.
   * Raw manual execution of migration `0013` alone is likewise incomplete. The canonical runner grants only thirteen controlled P02-M10-A Capture Session commands/projections, keeps every capture helper owner-only, preserves predecessor APIs, grants no direct table or sequence authority, and creates no media-scanning, hashing, transfer, provider, or production capability.
   * Raw manual execution of migration `0014` alone is likewise incomplete. The canonical runner grants only fourteen controlled P02-M11-A Cull Workspace commands/projections, keeps every cull helper owner-only, preserves predecessor APIs, grants no direct table or sequence authority, and creates no scanner, renderer, thumbnail, proxy, provider, editor-handoff, delivery, or production capability.
   * Raw manual execution of migration `0015` alone is likewise incomplete. The canonical runner grants only nine controlled P02-M12-A editor-handoff and returned-media commands/projections, keeps every packet helper owner-only, preserves predecessor APIs, grants no direct table or sequence authority, and creates no provider, media-processing, review, finalization, publication, delivery, or production capability.
   * Raw manual execution of migration `0016` alone is likewise incomplete. The canonical runner grants only thirteen controlled P02-M13-A review commands/projections, keeps every review helper owner-only, preserves predecessor APIs and final-source designations, grants no direct table or sequence authority, and creates no provider, Quick Edit execution, byte movement, publication, delivery, or production capability.
   * Raw manual execution of migration `0017` alone is likewise incomplete. The canonical runner grants only twelve controlled P02-M14-A publication, entitlement, grant, and projection functions, keeps every packet helper owner-only, preserves predecessor APIs, grants no direct table or sequence authority, and creates no provider operation, URL, byte movement, download execution, payment settlement, notification, or production capability.
   * Raw manual execution of migration `0018` alone is likewise incomplete. The canonical runner grants only seven controlled P02-M15-A Temporary Download Center commands/projections, keeps every packet helper owner-only, preserves predecessor APIs, grants no direct table or sequence authority, denies the restricted runtime all activity-observation DML, and creates no bearer/public credential, URL, recipient access, download endpoint, provider operation, payment settlement, notification, or production capability.
   * Raw manual execution of migration `0019` alone is likewise incomplete. The canonical runner grants only six P02-M15-B credential/gateway commands and projections, keeps all helpers owner-only, grants no direct packet-table or observation-table DML, permits only the SECURITY DEFINER gateway to append bounded observations, and creates no public route/page, URL, storage access, actual download, provider, notification, payment, customer, deployment, or production capability.
   * Raw manual execution of migration `0020` alone is likewise incomplete. The canonical runner grants only the P02-M15-C safe delivery-manifest projection, keeps direct table DML unavailable to the restricted runtime, reuses the M15-B gateway for every OPEN decision, and creates no table, provider/storage adapter, public or signed URL, real media delivery, customer/production access, payment, notification, deployment, or production capability.
   * Raw manual execution of migration `0021` alone is likewise incomplete. The canonical runner grants only the P02-M15-D internal file-backed delivery-source resolver, keeps direct table DML unavailable to the restricted runtime, reuses exactly one M15-B DOWNLOAD gateway attempt, and creates no table, provider credential, network retrieval, public/signed URL, real customer media delivery, deployment, or production capability.
   * Raw manual execution of migration `0022` alone is likewise incomplete. The canonical runner grants only the session-derived personal-summary share and revoke commands, keeps internal projection/export functions owner-only, grants no direct packet-table or sequence DML, and creates no customer-facing dashboard/export authority, trusted-billing activation, payment settlement truth, media/Property Hub access, external file, provider, deployment, or production capability.
   * Raw manual execution of migration `0026` alone is likewise incomplete. The canonical runner grants only seven controlled technical-observation and editorial-segment commands/projections, keeps validation and trigger helpers owner-only, grants no direct table or sequence DML, and creates no media scan, rename, trim, copy, upload, archive, handoff, deletion, provider, deployment, or production capability.
