...
5. **Domain Migration Ownership**
   ...
   * `0019_temporary_download_center_access_credential_gateway_foundation.sql` is the bounded P02-M15-B additive migration for one-time server-generated access secrets, nonreversible verifier evidence, rotation/revocation, shared current-policy evaluation, anonymous OPEN/DOWNLOAD authorization decisions, and the trusted append-only observation writer boundary.
