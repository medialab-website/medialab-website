...
export async function resetTestDatabase(options: ResetTestDatabaseOptions = {}): Promise<void> {
  const host = options.host || process.env.PGHOST || '/tmp/mlvs01-p02m14a-pg';
  const port = options.port || (process.env.PGPORT ? parseInt(process.env.PGPORT, 10) : 55443);
  const database = options.database || process.env.PGDATABASE || 'medialab_p02m14a_test';
  const confirm = options.confirm || process.env.CONFIRM_DATABASE || '';
  const user = options.user || process.env.PGUSER || 'medialab_p02m14a_test_owner';
  const runtimeUser = options.runtimeUser || process.env.PGRUNTIMEUSER || 'medialab_p02m14a_test_app';

  // Guard 1: Strict target database check
  if (database !== 'medialab_p02m14a_test') {
    throw new Error(`TEST_RESET_GUARD_FAILURE: Reset refused. Target database '${database}' is not the approved test database 'medialab_p02m14a_test'.`);
  }

  // Guard 2: Explicit confirmation check
  if (confirm !== 'medialab_p02m14a_test') {
    throw new Error(`TEST_RESET_GUARD_FAILURE: Reset refused. Missing or invalid confirmation '${confirm}'. Expected 'medialab_p02m14a_test'.`);
  }

  // Guard 3: Connection host & port check
  if (host !== '/tmp/mlvs01-p02m14a-pg') {
    throw new Error(`TEST_RESET_GUARD_FAILURE: Reset refused. Unapproved host '${host}'. Reset must use Unix socket '/tmp/mlvs01-p02m14a-pg'.`);
  }
  ...
  // Guard 4: Role check
  if (user !== 'medialab_p02m14a_test_owner') {
    throw new Error(`TEST_RESET_GUARD_FAILURE: Reset refused. Unapproved user '${user}'. Reset must use owner role 'medialab_p02m14a_test_owner'.`);
  }
  if (runtimeUser !== 'medialab_p02m14a_test_app') {
    throw new Error(`TEST_RESET_GUARD_FAILURE: Reset refused. Unapproved runtime role '${runtimeUser}'. Expected 'medialab_p02m14a_test_app'.`);
  }
...
if (scriptPath && currentPath === scriptPath) {
  ...
  resetTestDatabase({
    ...
    .then(() => {
      console.log('Test database reset and seed completed successfully for medialab_p02m14a_test.');
      process.exit(0);
    })
    .catch((err) => {
