...
import { MEDIA_ASSET_FOUNDATION_ROW_COUNT_INCREMENTS } from './fixtures/media-asset-identity-lineage-fixtures.js';
...
import { MEDIA_RETURN_REVIEW_FOUNDATION_ROW_COUNT_INCREMENTS } from './fixtures/returned-editor-review-final-source-fixtures.js';
import { PUBLICATION_DELIVERY_FOUNDATION_ROW_COUNT_INCREMENTS } from './fixtures/publication-delivery-entitlement-fixtures.js';
...
for (const [table, count] of Object.entries(TEMPORARY_DOWNLOAD_CENTER_ACCESS_CREDENTIAL_GATEWAY_FOUNDATION_ROW_COUNT_INCREMENTS)) {
  EXPECTED_RESET_ROW_COUNTS[table] = (EXPECTED_RESET_ROW_COUNTS[table] ?? 0) + count;
}
...
export async function resetTestDatabase(options: ResetTestDatabaseOptions = {}): Promise<void> {
  ...
  const runtimeUser = options.runtimeUser || process.env.PGRUNTIMEUSER || 'medialab_p02m15b_test_app';

  // Guard 1: Strict target database check
  if (database !== 'medialab_p02m15b_test') {
    throw new Error(`TEST_RESET_GUARD_FAILURE: Reset refused. Target database '${database}' is not the approved test database 'medialab_p02m15b_test'.`);
  }

  // Guard 2: Explicit confirmation check
  if (confirm !== 'medialab_p02m15b_test') {
    throw new Error(`TEST_RESET_GUARD_FAILURE: Reset refused. Missing or invalid confirmation '${confirm}'. Expected 'medialab_p02m15b_test'.`);
  }

  // Guard 3: Connection host & port check
  if (host !== '/tmp/mlvs01-p02m15b-pg') {
    throw new Error(`TEST_RESET_GUARD_FAILURE: Reset refused. Unapproved host '${host}'. Reset must use Unix socket '/tmp/mlvs01-p02m15b-pg'.`);
  }

  if (port !== 55443) {
    throw new Error(`TEST_RESET_GUARD_FAILURE: Reset refused. Unapproved port ${port}. Must be 55443.`);
  }

  // Guard 4: Role check
  if (user !== 'medialab_p02m15b_test_owner') {
    throw new Error(`TEST_RESET_GUARD_FAILURE: Reset refused. Unapproved user '${user}'. Reset must use owner role 'medialab_p02m15b_test_owner'.`);
  }
  if (runtimeUser !== 'medialab_p02m15b_test_app') {
    throw new Error(`TEST_RESET_GUARD_FAILURE: Reset refused. Unapproved runtime role '${runtimeUser}'. Expected 'medialab_p02m15b_test_app'.`);
  }
...
if (scriptPath && currentPath === scriptPath) {
  ...
  resetTestDatabase({
    ...
    .then(() => {
      console.log('Test database reset and seed completed successfully for medialab_p02m15b_test.');
      process.exit(0);
    })
    .catch((err) => {
