...
export async function runSeed(options: SeedOptions = {}): Promise<SeedResult> {
  ...
  if (host !== '/tmp/mlvs01-p02m08a-pg') {
    throw new Error(`SEED_SAFETY_FAILURE: Unapproved connection host '${host}'. Seed must use Unix socket '/tmp/mlvs01-p02m08a-pg'.`);
  }

  if (port !== 55438) {
    throw new Error(`SEED_SAFETY_FAILURE: Unapproved port ${port}. Must be 55438.`);
  }

  // Guard 3: User role validation
  const expectedUser = database === 'medialab_p02m08a_test'
    ? 'medialab_p02m08a_test_owner'
    : 'medialab_p02m04a_owner';
...
if (scriptPath && currentPath === scriptPath) {
  ...
  const targetDb = process.env.PGDATABASE ||
    (isTestDb ? 'medialab_p02m08a_test' : 'medialab_p02m04a');
