...
console.log('Running verify-capture-session-ingest-custody-schema.ts...');
const baseDir = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..');
const migrationDir = path.join(baseDir, 'db/migrations');
const packetMigration = '0013_capture_session_ingest_custody_foundation.sql';
const successorMigration = '0014_media_cull_workspace_selected_media_evidence_foundation.sql';
const secondSuccessorMigration = '0015_editor_handoff_returned_media_intake_foundation.sql';
const thirdSuccessorMigration = '0016_returned_editor_review_final_source_decision_foundation.sql';
const fourthSuccessorMigration = '0017_publication_delivery_entitlement_foundation.sql';
const fifthSuccessorMigration = '0018_temporary_download_center_external_sharing_foundation.sql';
...
const tables = ['capture_duplicate_evidence', 'capture_item_custody_events', 'capture_item_observations',
  'capture_item_promotions', 'capture_item_verification_events', 'capture_items', 'capture_session_assignments',
  'capture_session_events', 'capture_sessions', 'capture_source_observations', 'capture_sources'];
const publicFunctions = ['assign_capture_session', 'create_capture_session', 'get_capture_item_record',
  'get_capture_session_record', 'list_capture_sessions', 'promote_capture_item', 'record_capture_duplicate_evidence',
...
const helperFunctions = ['capture_session_assignment', 'reject_capture_evidence_mutation',
  'require_capture_permission', 'validate_capture_safe_json'];
const triggers = tables.map((table) => table === 'capture_item_verification_events' ? 'capture_item_verification_immutability_guard'
  : table === 'capture_item_custody_events' ? 'capture_item_custody_immutability_guard'
  : `${table}_immutability_guard`);
let errors = false;
...
exact('Migration inventory', fs.readdirSync(migrationDir).filter((name) => name.endsWith('.sql')),
  [...predecessors.map(([name]) => name), packetMigration, successorMigration, secondSuccessorMigration, thirdSuccessorMigration, fourthSuccessorMigration, fifthSuccessorMigration, sixthSuccessorMigration]);
...
for (const evidence of ['UNASSIGNED_QUARANTINE', 'QUARANTINE_ASSIGNMENT', 'EXACT_BYTE_MATCH',
  'REDUNDANT_CUSTODY_COPY', 'POSSIBLE_DUPLICATE', 'PROMOTED_TO_CANONICAL_ASSET', "'ORIGINAL'",
  'media_capture.manage', 'media_capture.read', 'media_asset.manage', 'check_media_idempotency', 'SECURITY DEFINER',
  'SET search_path = pg_catalog, medialab_core, pg_temp', 'Credentials, signed references, URLs']) {
  if (!sql.includes(evidence)) fail(`Migration 0013 missing required evidence: ${evidence}`);
}
for (const prohibited of ['ON DELETE CASCADE', 'CREATE TABLE medialab_core.media_assets',
  'CREATE TABLE medialab_core.media_storage_objects', 'CREATE TABLE medialab_core.media_operations',
  'drive.googleapis.com', 'dropbox.com', 'box.com', 'aws_secret_access_key']) {
  if (sql.includes(prohibited)) fail(`Migration 0013 contains prohibited evidence: ${prohibited}`);
}

const client = new pg.Client({ host: '/tmp/mlvs01-p02m15b-pg', port: 55443,
  database: 'medialab_p02m15b_test', user: 'medialab_p02m15b_test_owner' });
try {
  ...
  if (JSON.stringify(ledger.rows) !== JSON.stringify(expectedLedger)) fail('Eighteen-row migration ledger mismatch');
  ...
  if (dbTables.rows.some((row) => row.tableowner !== 'medialab_p02m15b_test_owner')) fail('Packet table ownership mismatch');
  const dbFunctions = await client.query(`SELECT p.proname,pg_get_userbyid(p.proowner) AS owner,p.prosecdef,p.proconfig,
    ...
    ['medialab_p02m15b_test_app', [...publicFunctions, ...helperFunctions]]);
  exact('Database function inventory', dbFunctions.rows.map((row) => row.proname), [...publicFunctions, ...helperFunctions]);
  if (dbFunctions.rows.some((row) => row.owner !== 'medialab_p02m15b_test_owner')) fail('Packet function ownership mismatch');
  ...
  if (dbFunctions.rows.some((row) => row.prosecdef && JSON.stringify(row.proconfig) !==
      JSON.stringify(['search_path=pg_catalog, medialab_core, pg_temp']))) fail('A SECURITY DEFINER function lacks the hardened search path');
  const dbTriggers = await client.query(`SELECT tgname FROM pg_trigger t JOIN pg_class c ON c.oid=t.tgrelid
    JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='medialab_core' AND c.relname=ANY($1::text[])
      AND NOT t.tgisinternal ORDER BY tgname`, [tables]);
  exact('Append-only trigger inventory', dbTriggers.rows.map((row) => row.tgname), triggers);
  const privileges = await client.query(`SELECT c.relname,
    ...
    WHERE n.nspname='medialab_core' AND c.relname=ANY($2::text[])`, ['medialab_p02m15b_test_app', tables]);
  ...
  const permissions = await client.query("SELECT code FROM medialab_core.permissions WHERE code LIKE 'media_capture.%' ORDER BY code");
  exact('Permission fixture inventory', permissions.rows.map((row) => row.code), ['media_capture.manage', 'media_capture.read']);
  const empty = await client.query(`SELECT sum(n)::int AS count FROM (${tables.map((table) =>
    `SELECT count(*) AS n FROM medialab_core.${table}`).join(' UNION ALL ')}) q`);
  if (empty.rows[0].count !== 0) fail('Capture fixture boundary contains operational rows');
} catch (error: any) {
  fail(`Database verification failed: ${error.message || String(error)}`);
} finally { await client.end().catch(() => undefined); }
if (errors) { console.error('Capture Session, ingest, and custody verification FAILED.'); process.exit(1); }
console.log(`Capture Session, ingest, and custody verification PASSED. 0013 SHA-256: ${packetHash}`);
