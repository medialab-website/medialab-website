...
exact('Migration inventory', fs.readdirSync(migrationDir).filter((name) => name.endsWith('.sql')),
  [...predecessors.map(([name]) => name), packetMigration, successorMigration, secondSuccessorMigration]);
...
const client = new pg.Client({ host: '/tmp/mlvs01-p02m12a-pg', port: 55442,
  database: 'medialab_p02m12a_test', user: 'medialab_p02m12a_test_owner' });
try {
  ...
  if (dbTables.rows.some((row) => row.tableowner !== 'medialab_p02m12a_test_owner')) fail('Packet table ownership mismatch');
  const dbFunctions = await client.query(`SELECT p.proname,pg_get_userbyid(p.proowner) AS owner,p.prosecdef,p.proconfig,
    ...
    ['medialab_p02m12a_test_app', [...publicFunctions, ...helperFunctions]]);
  exact('Database function inventory', dbFunctions.rows.map((row) => row.proname), [...publicFunctions, ...helperFunctions]);
  if (dbFunctions.rows.some((row) => row.owner !== 'medialab_p02m12a_test_owner')) fail('Packet function ownership mismatch');
  ...
  const privileges = await client.query(`SELECT c.relname,
    ...
    WHERE n.nspname='medialab_core' AND c.relname=ANY($2::text[])`, ['medialab_p02m12a_test_app', tables]);
