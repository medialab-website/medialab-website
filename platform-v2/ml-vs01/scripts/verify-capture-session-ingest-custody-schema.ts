...
exact('Migration inventory', fs.readdirSync(migrationDir).filter((name) => name.endsWith('.sql')),
  [...predecessors.map(([name]) => name), packetMigration, successorMigration, secondSuccessorMigration, thirdSuccessorMigration]);
...
try {
  ...
  const dbFunctions = await client.query(`SELECT p.proname,pg_get_userbyid(p.proowner) AS owner,p.prosecdef,p.proconfig,
    ...
    ['medialab_p02m13a_test_app', [...publicFunctions, ...helperFunctions]]);
  ...
  const privileges = await client.query(`SELECT c.relname,
    ...
    WHERE n.nspname='medialab_core' AND c.relname=ANY($2::text[])`, ['medialab_p02m13a_test_app', tables]);
