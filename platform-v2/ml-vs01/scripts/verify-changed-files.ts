...
for (const state of states) {
  ...
  const allowed = [
    ...
    "src/operations-console/operations-contracts.ts",
    ...
    "tests/operations-production-bridge.test.ts",
  ].includes(relative);
  if (!allowed) failures.push(`'${state.path}' is outside the P02-M21-A exact file boundary`);
}
if (actualPaths.length === 0) failures.push("candidate has no changed paths");

const result = {
  verifier: "P02-M21-A_CHANGED_FILES_V1",
