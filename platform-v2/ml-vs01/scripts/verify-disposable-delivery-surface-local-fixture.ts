...
try {
  ...
  if (ledger.rows.length !== 24 || ledger.rows[19].filename !== migration || ledger.rows[19].sha256 !== migrationHash || ledger.rows[20].filename !== '0021_provider_neutral_file_backed_disposable_delivery_foundation.sql' || ledger.rows[22].filename !== '0023_runtime_intake_reconciliation_commands.sql') fail('migration ledger identity mismatch');
