...
exact('Migration inventory',fs.readdirSync(migrationDir).filter(x=>x.endsWith('.sql')),[...predecessors.map(x=>x[0]),packetMigration]);
...
try{
  ...
  if(dbFunctions.rows.some(r=>r.owner!=='medialab_p02m12a_test_owner'||r.public||publicFunctions.includes(r.proname)!==r.runtime))
