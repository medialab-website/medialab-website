...
exact('Migration inventory',fs.readdirSync(migrationDir).filter(x=>x.endsWith('.sql')),[...predecessors.map(x=>x[0]),packetMigration,successorMigration,secondSuccessorMigration,thirdSuccessorMigration]);
...
const client=new pg.Client({host:'/tmp/mlvs01-p02m15a-pg',port:55443,database:'medialab_p02m15a_test',user:'medialab_p02m15a_test_owner'});
try{
  ...
  const expected=[...predecessors.map(([filename,sha256])=>({filename,sha256})),{filename:packetMigration,sha256:packetHash},{filename:successorMigration,sha256:successorHash},{filename:secondSuccessorMigration,sha256:secondSuccessorHash},{filename:thirdSuccessorMigration,sha256:thirdSuccessorHash}];
  ...
  if(dbTables.rows.some(r=>r.tableowner!=='medialab_p02m15a_test_owner'))fail('Packet table ownership mismatch');
  const dbFunctions=await client.query(`SELECT p.proname,pg_get_userbyid(p.proowner) owner,p.prosecdef,p.proconfig,
    ...
    AND p.proname=ANY($2::text[]) ORDER BY p.proname`,['medialab_p02m15a_test_app',[...publicFunctions,...helpers]]);
  exact('Database functions',dbFunctions.rows.map(r=>r.proname),[...publicFunctions,...helpers]);
  if(dbFunctions.rows.some(r=>r.owner!=='medialab_p02m15a_test_owner'||r.public||publicFunctions.includes(r.proname)!==r.runtime))
  ...
  const authority=await client.query(`SELECT count(*)::int n FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace
    ...
    (has_table_privilege($1,c.oid,'SELECT,INSERT,UPDATE,DELETE') OR has_table_privilege('public',c.oid,'SELECT,INSERT,UPDATE,DELETE'))`,['medialab_p02m15a_test_app']);
