...
const packetMigration='0015_editor_handoff_returned_media_intake_foundation.sql';
const successorMigration='0016_returned_editor_review_final_source_decision_foundation.sql';
const secondSuccessorMigration='0017_publication_delivery_entitlement_foundation.sql';
const thirdSuccessorMigration='0018_temporary_download_center_external_sharing_foundation.sql';
const fourthSuccessorMigration='0019_temporary_download_center_access_credential_gateway_foundation.sql';
const fifthSuccessorMigration='0020_disposable_delivery_surface_local_fixture_foundation.sql';
...
const tables=['editor_handoff_batches','editor_handoff_current','editor_handoff_events','editor_handoff_item_current',
  'editor_handoff_items','returned_media_intake_batches','returned_media_intake_events','returned_media_item_current',
  'returned_media_items','returned_media_match_events'];
const publicFunctions=['complete_editor_handoff_returns','create_editor_handoff_batch','create_returned_media_intake_batch',
  'get_editor_handoff_batch','get_returned_media_history','list_editor_handoff_batches','record_editor_handoff_event',
  'record_returned_media_item','resolve_returned_media_match'];
const helpers=['create_returned_version_for_match','recompute_editor_handoff_current','reject_editor_handoff_evidence_mutation',
  'require_editor_handoff_permission','validate_editor_handoff_reason','validate_editor_handoff_safe_json'];
const triggerTables=['editor_handoff_batches','editor_handoff_events','editor_handoff_items','returned_media_intake_batches',
  'returned_media_intake_events','returned_media_items','returned_media_match_events'];
let errors=false;
...
function exact(label:string,actual:string[],expected:string[]){
  if(JSON.stringify([...actual].sort())!==JSON.stringify([...expected].sort()))fail(`${label} mismatch. Expected ${[...expected].sort().join(', ')}, got ${[...actual].sort().join(', ')}`);
}
...
exact('Migration inventory',fs.readdirSync(migrationDir).filter(x=>x.endsWith('.sql')),[...predecessors.map(x=>x[0]),packetMigration,successorMigration,secondSuccessorMigration,thirdSuccessorMigration,fourthSuccessorMigration,fifthSuccessorMigration,sixthSuccessorMigration]);
...
for(const required of ['PHOTO','VIDEO','EDITOR_HANDOFF','EDITOR_RETURN','REVISION_RETURN','ORIGINAL_TO_EDITOR_RETURN',
  'EXACT_MATCH','CONFIDENT_MATCH','AMBIGUOUS','UNMATCHED','CONFLICTING','REPEATED_RETURN','FILENAME_ONLY',
  'media_editor_handoff.manage','media_editor_handoff.read','media_asset.manage','FOR UPDATE','SECURITY DEFINER',
  'SET search_path = pg_catalog, medialab_core, pg_temp'])if(!sql.includes(required))fail(`Missing required evidence ${required}`);
for(const prohibited of ['ON DELETE CASCADE','CREATE TABLE medialab_core.media_assets','CREATE TABLE medialab_core.media_operations',
  'CREATE TABLE medialab_core.media_storage_objects','drive.googleapis.com','pixelmob.com','aws_secret_access_key'])
  if(sql.includes(prohibited))fail(`Prohibited migration evidence ${prohibited}`);
const client=new pg.Client({host:'/tmp/mlvs01-p02m15d-pg',port:55445,database:'medialab_p02m15d_test',user:'medialab_p02m15d_test_owner'});
try{
  ...
  const expected=[...predecessors.map(([filename,sha256])=>({filename,sha256})),{filename:packetMigration,sha256:packetHash},{filename:successorMigration,sha256:successorHash},{filename:secondSuccessorMigration,sha256:secondSuccessorHash},{filename:thirdSuccessorMigration,sha256:thirdSuccessorHash},{filename:fourthSuccessorMigration,sha256:fourthSuccessorHash},{filename:fifthSuccessorMigration,sha256:fifthSuccessorHash},{filename:sixthSuccessorMigration,sha256:sixthSuccessorHash}];
  if(JSON.stringify(ledger.rows)!==JSON.stringify(expected))fail('Twenty-one-row migration ledger mismatch');
  ...
  if(dbTables.rows.some(r=>r.tableowner!=='medialab_p02m15d_test_owner'))fail('Packet table ownership mismatch');
  const dbFunctions=await client.query(`SELECT p.proname,pg_get_userbyid(p.proowner) owner,p.prosecdef,p.proconfig,
    ...
    AND p.proname=ANY($2::text[]) ORDER BY p.proname`,['medialab_p02m15d_test_app',[...publicFunctions,...helpers]]);
  exact('Database functions',dbFunctions.rows.map(r=>r.proname),[...publicFunctions,...helpers]);
  if(dbFunctions.rows.some(r=>r.owner!=='medialab_p02m15d_test_owner'||r.public||publicFunctions.includes(r.proname)!==r.runtime))
    fail('Function ownership or grant mismatch');
  if(dbFunctions.rows.some(r=>r.prosecdef&&JSON.stringify(r.proconfig)!==JSON.stringify(['search_path=pg_catalog, medialab_core, pg_temp'])))
    fail('Unsafe SECURITY DEFINER search_path');
  const triggers=await client.query(`SELECT c.relname FROM pg_trigger t JOIN pg_class c ON c.oid=t.tgrelid
    ...
    AND (c.relname LIKE 'editor_handoff_%' OR c.relname LIKE 'returned_media_%') AND NOT t.tgisinternal`);
  ...
  const authority=await client.query(`SELECT count(*)::int n FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace
    WHERE n.nspname='medialab_core' AND (c.relname LIKE 'editor_handoff_%' OR c.relname LIKE 'returned_media_%') AND
    (has_table_privilege($1,c.oid,'SELECT,INSERT,UPDATE,DELETE') OR has_table_privilege('public',c.oid,'SELECT,INSERT,UPDATE,DELETE'))`,['medialab_p02m15d_test_app']);
  ...
  const permissions=await client.query("SELECT code FROM medialab_core.permissions WHERE code LIKE 'media_editor_handoff.%' ORDER BY code");
  exact('Permission inventory',permissions.rows.map(r=>r.code),['media_editor_handoff.manage','media_editor_handoff.read']);
  const seeded=await client.query(`SELECT sum(n)::int n FROM (SELECT count(*) n FROM medialab_core.editor_handoff_batches
    UNION ALL SELECT count(*) FROM medialab_core.editor_handoff_items UNION ALL SELECT count(*) FROM medialab_core.returned_media_intake_batches
    UNION ALL SELECT count(*) FROM medialab_core.returned_media_items) q`);
  if(seeded.rows[0].n!==0)fail('Fixtures contain editor handoff or returned-media operational rows');
}catch(error:any){fail(error.message||String(error));}finally{await client.end().catch(()=>undefined);}
if(errors)process.exit(1);
console.log(`Editor handoff and returned-media intake schema verification PASSED. 0015 SHA-256: ${packetHash}`);
