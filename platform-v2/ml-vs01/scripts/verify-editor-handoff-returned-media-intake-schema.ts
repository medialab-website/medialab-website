...
exact('Migration inventory',fs.readdirSync(migrationDir).filter(x=>x.endsWith('.sql')),[...predecessors.map(x=>x[0]),packetMigration,successorMigration]);
...
try{
  ...
  const expected=[...predecessors.map(([filename,sha256])=>({filename,sha256})),{filename:packetMigration,sha256:packetHash},{filename:successorMigration,sha256:successorHash}];
  ...
  if(dbFunctions.rows.some(r=>r.owner!=='medialab_p02m13a_test_owner'||r.public||publicFunctions.includes(r.proname)!==r.runtime))
