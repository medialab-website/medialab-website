...
const eleventhSuccessorMigration = '0020_disposable_delivery_surface_local_fixture_foundation.sql';
const twelfthSuccessorMigration = '0021_provider_neutral_file_backed_disposable_delivery_foundation.sql';
const thirteenthSuccessorMigration = '0022_organization_records_dashboard_audited_export_foundation.sql';
...
try {
  ...
  const expectedLedger = [
    ...
    {
      filename: seventhSuccessorMigration,
      sha256: crypto.createHash('sha256').update(fs.readFileSync(path.join(migrationDir, seventhSuccessorMigration))).digest('hex')
    },
    {
      filename: eighthSuccessorMigration,
      sha256: crypto.createHash('sha256').update(fs.readFileSync(path.join(migrationDir, eighthSuccessorMigration))).digest('hex')
    },
    {
    ...
    {
      filename: eleventhSuccessorMigration,
      sha256: crypto.createHash('sha256').update(fs.readFileSync(path.join(migrationDir, eleventhSuccessorMigration))).digest('hex')
    },
    {
      filename: twelfthSuccessorMigration,
      sha256: crypto.createHash('sha256').update(fs.readFileSync(path.join(migrationDir, twelfthSuccessorMigration))).digest('hex')
    },
    {
      filename: thirteenthSuccessorMigration,
      sha256: crypto.createHash('sha256').update(fs.readFileSync(path.join(migrationDir, thirteenthSuccessorMigration))).digest('hex')
    },
    {
      filename: runtimeIntakeSuccessorMigration,
      sha256: crypto.createHash('sha256').update(fs.readFileSync(path.join(migrationDir, runtimeIntakeSuccessorMigration))).digest('hex')
    },
    {
      filename: operationsSuccessorMigration,
      sha256: crypto.createHash('sha256').update(fs.readFileSync(path.join(migrationDir, operationsSuccessorMigration))).digest('hex')
    }
  ];
