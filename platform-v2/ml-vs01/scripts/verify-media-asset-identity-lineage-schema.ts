...
exact('Migration inventory', migrationFiles, [...predecessorMigrations.map(([name]) => name), packetMigration, successorMigration, secondSuccessorMigration, thirdSuccessorMigration]);
...
try {
  ...
  const privileges = await client.query(
    ...
    ['medialab_p02m11a_test_app', tables]
  );
