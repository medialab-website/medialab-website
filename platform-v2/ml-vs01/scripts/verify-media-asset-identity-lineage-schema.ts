...
exact('Migration inventory', migrationFiles, [...predecessorMigrations.map(([name]) => name), packetMigration, successorMigration, secondSuccessorMigration]);
...
try {
  ...
  const privileges = await client.query(
    ...
    ['medialab_p02m10a_test_app', tables]
  );
