...
exact('Migration inventory', migrationFiles, [...predecessorMigrations.map(([name]) => name), packetMigration]);
...
const client = new pg.Client({ host: '/tmp/mlvs01-p02m08a-pg', port: 55438,
  database: 'medialab_p02m08a_test', user: 'medialab_p02m08a_test_owner' });
try {
  ...
  if (dbTables.rows.some((row) => row.tableowner !== 'medialab_p02m08a_test_owner')) fail('Packet table ownership mismatch');
  const dbFunctions = await client.query(
    ...
    ['medialab_p02m08a_test_app', [...publicFunctions, ...helperFunctions]]
  );
  exact('Database function inventory', dbFunctions.rows.map((row) => row.proname), [...publicFunctions, ...helperFunctions]);
  if (dbFunctions.rows.some((row) => row.owner !== 'medialab_p02m08a_test_owner')) fail('Packet function ownership mismatch');
  ...
  const privileges = await client.query(
    ...
    ['medialab_p02m08a_test_app', tables]
  );
