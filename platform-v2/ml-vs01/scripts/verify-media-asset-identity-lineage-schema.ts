...
const migrationFiles = fs.readdirSync(migrationDir).filter((name) => name.endsWith('.sql') && name !== '0025_operations_mission_plan_draft_controls.sql' && name !== '0026_editorial_segment_foundation.sql' && name !== '0027_contextual_editor_review_mobile_quick_edit_web_bridge.sql' && name !== '0028_client_account_and_operator_contact_intake_foundation.sql').sort();
