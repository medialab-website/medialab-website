...
exact('Migration inventory',fs.readdirSync(migrationDir).filter(x=>x.endsWith('.sql') && x!=='0025_operations_mission_plan_draft_controls.sql'&&x!=='0026_editorial_segment_foundation.sql'&&x!=='0027_contextual_editor_review_mobile_quick_edit_web_bridge.sql'&&x!=='0028_client_account_and_operator_contact_intake_foundation.sql'),
...
try {
  ...
  if (dbFunctions.rows.some(r=>r.owner!=='medialab_p02m16a_test_owner'||r.public||publicFunctions.includes(r.proname)!==r.runtime)) fail('Function ownership or grant mismatch');
