...
exact('Migration inventory',fs.readdirSync(migrationDir).filter(x=>x.endsWith('.sql')),
  [...predecessors.map(x=>x[0]),packetMigration,successorMigration,secondSuccessorMigration,thirdSuccessorMigration,fourthSuccessorMigration]);
...
try {
  ...
  if (dbFunctions.rows.some(r=>r.owner!=='medialab_p02m15a_test_owner'||r.public||publicFunctions.includes(r.proname)!==r.runtime)) fail('Function ownership or grant mismatch');
