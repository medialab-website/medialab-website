...
exact('Migration inventory',fs.readdirSync(migrationDir).filter(x=>x.endsWith('.sql')),
  [...predecessors.map(x=>x[0]),packetMigration]);
...
const client=new pg.Client({host:'/tmp/mlvs01-p02m11a-pg',port:55441,database:'medialab_p02m11a_test',user:'medialab_p02m11a_test_owner'});
try {
  ...
  if (JSON.stringify(ledger.rows)!==JSON.stringify(expected)) fail('Fourteen-row migration ledger mismatch');
  ...
  if (dbTables.rows.some(r=>r.tableowner!=='medialab_p02m11a_test_owner')) fail('Packet table ownership mismatch');
  const dbFunctions=await client.query(`SELECT p.proname,pg_get_userbyid(p.proowner) owner,p.prosecdef,p.proconfig,
    ...
    AND p.proname=ANY($2::text[]) ORDER BY p.proname`,['medialab_p02m11a_test_app',[...publicFunctions,...helpers]]);
  exact('Database functions',dbFunctions.rows.map(r=>r.proname),[...publicFunctions,...helpers]);
  if (dbFunctions.rows.some(r=>r.owner!=='medialab_p02m11a_test_owner'||r.public||publicFunctions.includes(r.proname)!==r.runtime)) fail('Function ownership or grant mismatch');
  ...
  const authority=await client.query(`SELECT count(*)::int n FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace
    ...
    (has_table_privilege($1,c.oid,'SELECT,INSERT,UPDATE,DELETE') OR has_table_privilege('public',c.oid,'SELECT,INSERT,UPDATE,DELETE'))`,['medialab_p02m11a_test_app']);
