...
const successorMigration = '0011_media_asset_identity_and_lineage_foundation.sql';
const secondSuccessorMigration = '0012_durable_media_operations_reconciliation_foundation.sql';
const thirdSuccessorMigration = '0013_capture_session_ingest_custody_foundation.sql';
const fourthSuccessorMigration = '0014_media_cull_workspace_selected_media_evidence_foundation.sql';
const fifthSuccessorMigration = '0015_editor_handoff_returned_media_intake_foundation.sql';
const sixthSuccessorMigration = '0016_returned_editor_review_final_source_decision_foundation.sql';
const seventhSuccessorMigration = '0017_publication_delivery_entitlement_foundation.sql';
const eighthSuccessorMigration = '0018_temporary_download_center_external_sharing_foundation.sql';
const ninthSuccessorMigration = '0019_temporary_download_center_access_credential_gateway_foundation.sql';
...
const migrationFiles = fs.readdirSync(migrationDir).filter((filename) => filename.endsWith('.sql') && filename !== '0025_operations_mission_plan_draft_controls.sql' && filename !== '0026_editorial_segment_foundation.sql' && filename !== '0027_contextual_editor_review_mobile_quick_edit_web_bridge.sql' && filename !== '0028_client_account_and_operator_contact_intake_foundation.sql').sort();
...
try {
  ...
  const expectedLedger = [
    ...
    { filename: successorMigration, sha256: crypto.createHash('sha256').update(fs.readFileSync(path.join(migrationDir, successorMigration))).digest('hex') },
    { filename: secondSuccessorMigration, sha256: crypto.createHash('sha256').update(fs.readFileSync(path.join(migrationDir, secondSuccessorMigration))).digest('hex') },
    { filename: thirdSuccessorMigration, sha256: crypto.createHash('sha256').update(fs.readFileSync(path.join(migrationDir, thirdSuccessorMigration))).digest('hex') },
    { filename: fourthSuccessorMigration, sha256: crypto.createHash('sha256').update(fs.readFileSync(path.join(migrationDir, fourthSuccessorMigration))).digest('hex') },
    { filename: fifthSuccessorMigration, sha256: crypto.createHash('sha256').update(fs.readFileSync(path.join(migrationDir, fifthSuccessorMigration))).digest('hex') },
    { filename: sixthSuccessorMigration, sha256: crypto.createHash('sha256').update(fs.readFileSync(path.join(migrationDir, sixthSuccessorMigration))).digest('hex') },
    { filename: seventhSuccessorMigration, sha256: crypto.createHash('sha256').update(fs.readFileSync(path.join(migrationDir, seventhSuccessorMigration))).digest('hex') },
    { filename: eighthSuccessorMigration, sha256: crypto.createHash('sha256').update(fs.readFileSync(path.join(migrationDir, eighthSuccessorMigration))).digest('hex') },
    { filename: ninthSuccessorMigration, sha256: crypto.createHash('sha256').update(fs.readFileSync(path.join(migrationDir, ninthSuccessorMigration))).digest('hex') },
    { filename: tenthSuccessorMigration, sha256: crypto.createHash('sha256').update(fs.readFileSync(path.join(migrationDir, tenthSuccessorMigration))).digest('hex') },
    { filename: eleventhSuccessorMigration, sha256: crypto.createHash('sha256').update(fs.readFileSync(path.join(migrationDir, eleventhSuccessorMigration))).digest('hex') },
    { filename: twelfthSuccessorMigration, sha256: crypto.createHash('sha256').update(fs.readFileSync(path.join(migrationDir, twelfthSuccessorMigration))).digest('hex') },
