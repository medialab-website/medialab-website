...
if (migrationFiles.length !== 26 || migrationFiles.some((name, index) => !name.startsWith(String(index + 1).padStart(4, "0"))) || migrationFiles[22] !== "0023_runtime_intake_reconciliation_commands.sql" || migrationFiles[23] !== "0024_operations_home_scheduling_assignment_console.sql" || migrationFiles[24] !== "0025_operations_mission_plan_draft_controls.sql" || migrationFiles[25] !== "0026_editorial_segment_foundation.sql") fail("migration ledger is not exactly 0001-0026");
