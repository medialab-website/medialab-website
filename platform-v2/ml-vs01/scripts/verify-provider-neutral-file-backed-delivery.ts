...
try {
  ...
  if (ledger.rows.length !== 28 || ledger.rows[20].filename !== migration || ledger.rows[20].sha256 !== migrationSha256 || ledger.rows[22].filename !== '0023_runtime_intake_reconciliation_commands.sql') fail('migration ledger identity mismatch');
