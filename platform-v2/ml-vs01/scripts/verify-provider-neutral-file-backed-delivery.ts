...
try {
  ...
  if (ledger.rows.length !== 21 || ledger.rows[20].filename !== migration || ledger.rows[20].sha256 !== migrationSha256) fail('migration ledger identity mismatch');
  const fn = await client.query(`SELECT p.prosecdef,p.proconfig,pg_get_userbyid(p.proowner) owner,
    ...
    WHERE n.nspname='medialab_core' AND p.proname='resolve_temporary_download_center_delivery_source'`, ['medialab_p02m15d_test_app']);
  ...
  const runtimeDml = await client.query(`SELECT count(*)::int n FROM information_schema.role_table_grants WHERE grantee=$1 AND table_schema='medialab_core' AND privilege_type IN ('INSERT','UPDATE','DELETE')`, ['medialab_p02m15d_test_app']);
