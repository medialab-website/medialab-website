...
const client=new pg.Client({host:'/tmp/mlvs01-p02m15e-pg',port:55446,database:'medialab_p02m15e_test',user:'medialab_p02m15e_test_owner'});await client.connect();
try{
  const ledger=await client.query('SELECT filename,sha256 FROM medialab_meta.schema_migrations ORDER BY filename');if(ledger.rows.length !== 22||ledger.rows[16].filename!==migration||ledger.rows[16].sha256!==packetHash||ledger.rows[17].filename!=='0018_temporary_download_center_external_sharing_foundation.sql'||ledger.rows[18].filename!=='0019_temporary_download_center_access_credential_gateway_foundation.sql'||ledger.rows[19].filename!=='0020_disposable_delivery_surface_local_fixture_foundation.sql'||ledger.rows[20].filename!=='0021_provider_neutral_file_backed_disposable_delivery_foundation.sql')fail('migration ledger identity mismatch');
  const t=await client.query("SELECT tablename,tableowner FROM pg_tables WHERE schemaname='medialab_core' AND tablename=ANY($1::text[]) ORDER BY tablename",[tables]);if(JSON.stringify(t.rows.map(r=>r.tablename))!==JSON.stringify(tables)||t.rows.some(r=>r.tableowner!=='medialab_p02m15e_test_owner'))fail('table inventory or owner mismatch');
  const f=await client.query(`SELECT p.proname,pg_get_userbyid(p.proowner) owner,has_function_privilege($1,p.oid,'EXECUTE') runtime,has_function_privilege('public',p.oid,'EXECUTE') public FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace WHERE n.nspname='medialab_core' AND p.proname=ANY($2::text[]) ORDER BY p.proname`,['medialab_p02m15e_test_app',[...runtimeFunctions,...helpers]]);if(f.rows.length!==runtimeFunctions.length+helpers.length||f.rows.some(r=>r.owner!=='medialab_p02m15e_test_owner'||r.public||r.runtime!==runtimeFunctions.includes(r.proname)))fail('function owner/runtime/PUBLIC grant mismatch');
  const dml=await client.query(`SELECT c.relname,(has_table_privilege($1,c.oid,'SELECT') OR has_table_privilege($1,c.oid,'INSERT') OR has_table_privilege($1,c.oid,'UPDATE') OR has_table_privilege($1,c.oid,'DELETE')) runtime,(has_table_privilege('public',c.oid,'SELECT') OR has_table_privilege('public',c.oid,'INSERT') OR has_table_privilege('public',c.oid,'UPDATE') OR has_table_privilege('public',c.oid,'DELETE')) public FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='medialab_core' AND c.relname=ANY($2::text[])`,['medialab_p02m15e_test_app',tables]);if(dml.rows.some(r=>r.runtime||r.public))fail('runtime or PUBLIC table authority detected');
