...
const pgDataDir = '/tmp/mlvs01-p02m15a-data';
...
const socketDir = '/tmp/mlvs01-p02m15a-pg';
...
try {
  execSync('/Applications/Postgres.app/Contents/Versions/latest/bin/pg_isready -h /tmp/mlvs01-p02m15a-pg -p 55443 -d medialab_p02m15a_test', { encoding: 'utf-8' });
  const identity = execSync(
    "/Applications/Postgres.app/Contents/Versions/latest/bin/psql -h /tmp/mlvs01-p02m15a-pg -p 55443 -U medialab_p02m15a_test_owner -d medialab_p02m15a_test -Atc \"SELECT current_database()||'|'||current_user\"",
    { encoding: 'utf-8' }
  ).trim();
  if (identity !== 'medialab_p02m15a_test|medialab_p02m15a_test_owner') {
