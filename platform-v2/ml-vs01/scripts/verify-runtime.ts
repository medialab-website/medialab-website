...
const pgDataDir = '/tmp/mlvs01-p02m15c-data';
...
const socketDir = '/tmp/mlvs01-p02m15c-pg';
...
try {
  execSync('/Applications/Postgres.app/Contents/Versions/latest/bin/pg_isready -h /tmp/mlvs01-p02m15c-pg -p 55444 -d medialab_p02m15c_test', { encoding: 'utf-8' });
  const identity = execSync(
    "/Applications/Postgres.app/Contents/Versions/latest/bin/psql -h /tmp/mlvs01-p02m15c-pg -p 55444 -U medialab_p02m15c_test_owner -d medialab_p02m15c_test -Atc \"SELECT current_database()||'|'||current_user\"",
    { encoding: 'utf-8' }
  ).trim();
  if (identity !== 'medialab_p02m15c_test|medialab_p02m15c_test_owner') {
