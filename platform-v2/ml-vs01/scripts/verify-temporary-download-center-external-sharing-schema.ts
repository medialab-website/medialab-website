...
try {
  ...
  if (JSON.stringify(tableInventory.rows.map(row => row.tablename)) !== JSON.stringify(tables) || tableInventory.rows.some(row => row.tableowner !== 'medialab_p02m15e_test_owner')) fail('table inventory or owner mismatch');
  ...
  if (functions.rows.length !== runtimeFunctions.length + helpers.length || functions.rows.some(row => row.owner !== 'medialab_p02m15e_test_owner' || row.public || row.runtime !== runtimeFunctions.includes(row.proname))) fail('function owner/runtime/PUBLIC grant mismatch');
