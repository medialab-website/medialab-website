...
try {
  ...
  if (ledger.rows.length !== 20 || ledger.rows[17].filename !== migration || ledger.rows[17].sha256 !== packetHash || ledger.rows[18].filename !== '0019_temporary_download_center_access_credential_gateway_foundation.sql' || ledger.rows[19].filename !== '0020_disposable_delivery_surface_local_fixture_foundation.sql') fail('migration ledger identity mismatch');
  ...
  if (JSON.stringify(tableInventory.rows.map(row => row.tablename)) !== JSON.stringify(tables) || tableInventory.rows.some(row => row.tableowner !== 'medialab_p02m15c_test_owner')) fail('table inventory or owner mismatch');
  const functions = await client.query(`SELECT p.proname,pg_get_userbyid(p.proowner) owner,p.prosecdef,p.proconfig,pg_get_function_identity_arguments(p.oid) arguments,has_function_privilege($1,p.oid,'EXECUTE') runtime,has_function_privilege('public',p.oid,'EXECUTE') public FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace WHERE n.nspname='medialab_core' AND p.proname=ANY($2::text[]) ORDER BY p.proname`, ['medialab_p02m15c_test_app', [...runtimeFunctions, ...helpers]]);
  if (functions.rows.length !== runtimeFunctions.length + helpers.length || functions.rows.some(row => row.owner !== 'medialab_p02m15c_test_owner' || row.public || row.runtime !== runtimeFunctions.includes(row.proname))) fail('function owner/runtime/PUBLIC grant mismatch');
