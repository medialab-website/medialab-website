...
try {
  ...
  if (ledger.rows.length !== 18 || ledger.rows[17].filename !== migration || ledger.rows[17].sha256 !== packetHash) fail('migration ledger identity mismatch');
  ...
  if (JSON.stringify(tableInventory.rows.map(row => row.tablename)) !== JSON.stringify(tables) || tableInventory.rows.some(row => row.tableowner !== 'medialab_p02m15a_test_owner')) fail('table inventory or owner mismatch');
  ...
  if (functions.rows.length !== runtimeFunctions.length + helpers.length || functions.rows.some(row => row.owner !== 'medialab_p02m15a_test_owner' || row.public || row.runtime !== runtimeFunctions.includes(row.proname))) fail('function owner/runtime/PUBLIC grant mismatch');
