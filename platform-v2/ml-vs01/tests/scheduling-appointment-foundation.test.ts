...
describe('P02-M05-A provider-neutral Scheduling Request and Appointment foundation', () => {
  ...
  it('1. applies exactly nine immutable migrations and preserves predecessor checksums', async () => {
  ...
  it('2. creates the bounded scheduling inventory without calendars, crews, payroll, or notifications', async () => {
    ...
    const excluded = await owner.query(
      `SELECT tablename FROM pg_tables WHERE schemaname = 'medialab_core'
         AND (tablename LIKE 'calendar%' OR tablename LIKE 'crew%'
